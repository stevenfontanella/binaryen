...
std::unordered_set<Function*> getReferencedFuncs(Module& module,
  ...
  struct AddressedFuncsWalker : WalkerPass<PostWalker<AddressedFuncsWalker>> {
    ...
    // The key for `nullptr` contains references that are not in a function (e.g. `elem` segments).
    std::unordered_map<Function*, std::unordered_set<Function*>>& allReferencedFuncs;
    // Points to `allReferencedFuncs`.
    std::unordered_set<Function*>* referencedFuncs = nullptr;

    AddressedFuncsWalker(
      std::unordered_map<Function*, std::unordered_set<Function*>>& allReferencedFuncs)
      : allReferencedFuncs(allReferencedFuncs), referencedFuncs(&allReferencedFuncs[nullptr]) {}
    ...
    void doWalkFunction(Function* func) {
      referencedFuncs = &allReferencedFuncs[func];
      walk(func->body);
    }
